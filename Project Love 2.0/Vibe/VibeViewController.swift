...
class VibeViewController: UIViewController,UICollectionViewDelegate,MoodCheckInCellDelegate, TellMoodSelectionDelegate, DailyCheckInCellDelegate, SmallModalDelegate, InfoModalDelegate, UIAdaptivePresentationControllerDelegate, SuggestedActivitiesModalDelegate {
    ...
    // These three are stored in DataStore.shared so they persist across tab switches
    var hasCompletedDailyCheckIn: Bool {
        get { DataStore.shared.hasCompletedDailyCheckIn }
        set { DataStore.shared.hasCompletedDailyCheckIn = newValue }
    }
    var resolvedVibeTitle: VibeTitle? {
        get { DataStore.shared.resolvedVibeTitle }
        set { DataStore.shared.resolvedVibeTitle = newValue }
    }
    var hasAchievedNewVibe: Bool {
        get { DataStore.shared.hasAchievedNewVibe }
        set { DataStore.shared.hasAchievedNewVibe = newValue }
    }
    ...
    override func viewDidLoad() {
        ...
        // Only load suggestedActivities if the vibe hasn't been achieved yet
    ...
    func generateLayout() -> UICollectionViewLayout {
        ...
        let layout = UICollectionViewCompositionalLayout { section, env in
            let headerToItemsSpacing: CGFloat = 8
            let sectionTopSpacing: CGFloat = 24
            ...
            // Section 2: How are you feeling / Moods
                ...
                largeTitleHeader.edgeSpacing = NSCollectionLayoutEdgeSpacing(
                    leading: nil,
                    top: .fixed(26),
                    trailing: nil,
                    bottom: nil
                ...
                let backgroundItem = NSCollectionLayoutDecorationItem.background(elementKind: PlainWhiteBackgroundView.kind)
                backgroundItem.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 0, bottom: -1000, trailing: 0)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Task { [weak self] in
            ...
            await MainActor.run {
                ...
                // Sync suggested activities from DataStore — but never overwrite
                ...
                if !self.hasAchievedNewVibe {
                    ...
                    // Vibe already achieved — force suggestedActivities to empty
